...
const CanvasTreeVisualization = ({ data, stepInfo }) => {
  ...
  const colors = {
    ...
    node: {
      ...
      processing: '#FCD34D',
      highlighted: '#60A5FA',
      queue: '#FDBA74' // Color for nodes in the queue
    },
    border: {
      default: '#9CA3AF',
      visited: '#059669',
      processing: '#F59E0B',
      highlighted: '#2563EB',
      queue: '#F97316' // Border color for nodes in the queue
    },
    edge: {
      default: '#9CA3AF',
      highlighted: '#60A5FA'
  ...
  const drawNode = (ctx, position, value, status) => {
    ...
    if (status === 'processing') {
      ...
      fillColor = colors.node.queue;
      strokeColor = colors.border.queue;
    }
  ...
  const drawNodes = (ctx, node, positions) => {
    ...
    if (data.processing === node.id) {
      ...
      status = 'visited';
    } else if (data.queue.includes(node.id)) {
      status = 'queue';
...
const generateInitialData = () => {
  ...
  return {
    ...
    visited: [],
    processing: null,
    highlighted: [1], // Start with root highlighted
    queue: [], // Queue of nodes to be processed
    result: [],
  };
};

// Generate visualization steps for level order traversal (breadth first)
const generateVisualizationSteps = (initialData) => {
  ...
  const findNodeById = (tree, id) => {
    ...
    const leftResult = findNodeById(tree.left, id);
    if (leftResult) return leftResult;
    
    return findNodeById(tree.right, id);
  };
  ...
  const levelOrderTraversal = (tree) => {
    ...
    const queue = [tree]; // Start with just the root node
    const nodeIds = [tree.id]; // Track node IDs in the queue
    
    // First step - show root and empty queue
    steps.push({
      data: {
        ...initialData,
      ...
      description: `Starting level order traversal. Visit nodes level by level.`,
    });
    
    // Add root to queue step
    steps.push({
      data: {
        ...initialData,
        tree: initialData.tree,
      ...
      description: `Add root node ${tree.value} to the queue.`,
    });
    ...
    while (queue.length > 0) {
      ...
      steps.push({
        ...
        description: `Dequeue and process node ${node.value}.`,
      });
      ...
      steps.push({
        ...
        description: `Visit node ${node.value}. Current traversal: [${result.join(', ')}]`,
      });
      
      // Enqueue left child if exists
      if (node.left) {
        queue.push(node.left);
        nodeIds.push(node.left.id);
        ...
        steps.push({
          ...
          description: `Add left child ${node.left.value} of node ${node.value} to the queue.`,
        });
      }
      ...
      if (node.right) {
        ...
        steps.push({
          ...
          description: `Add right child ${node.right.value} of node ${node.value} to the queue.`,
        });
      }
      
      // Show queue after adding children
      if (node.left || node.right) {
        steps.push({
          ...
          description: `Queue now contains nodes: [${nodeIds.map(id => findNodeById(initialData.tree, id).value).join(', ')}]`,
        });
      }
    }
  };

  levelOrderTraversal(initialData.tree);
...
const LevelOrderTraversalVisualization = () => {
  return (
    <VisualizationContainer
      ...
      description="Visualize how level order traversal visits nodes in a binary tree level by level, from left to right."
    />
  );
};
