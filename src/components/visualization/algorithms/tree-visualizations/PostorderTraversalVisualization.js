...
// Generate visualization steps - modified for postorder traversal (left, right, root)
const generateVisualizationSteps = (initialData) => {
  ...
  const postorderTraversal = (node, path = []) => {
    ...
    if (node.left) {
      ...
      postorderTraversal(node.left, [...path, node.id]);
    }
    ...
    if (node.right) {
      ...
      postorderTraversal(node.right, [...path, node.id]);
    }
    ...
    steps.push({
      ...
      description: `Visiting node ${node.value} (root last). Current traversal: [${result.join(', ')}]`,
    });
  };

  postorderTraversal(initialData.tree);
...
const PostorderTraversalVisualization = () => {
  return (
    <VisualizationContainer
      ...
      description="Visualize how postorder traversal visits nodes in a binary tree in the order: left subtree, right subtree, root."
    />
  );
};
