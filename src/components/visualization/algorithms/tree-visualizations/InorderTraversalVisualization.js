...
const generateVisualizationSteps = (initialData) => {
  ...
  const inorderTraversal = (node, path = []) => {
    ...
    if (node.left) {
      ...
      inorderTraversal(node.left, [...path, node.id]);
    }
    ...
    steps.push({
      ...
      description: `Visiting node ${node.value}. Current traversal: [${result.join(', ')}]`,
    });
    ...
    if (node.right) {
      ...
      inorderTraversal(node.right, [...path, node.id]);
    }
  };

  inorderTraversal(initialData.tree);
...
const InorderTraversalVisualization = () => {
  return (
    <VisualizationContainer
      ...
      description="Visualize how inorder traversal visits nodes in a binary tree in the order: left subtree, root, right subtree."
    />
  );
};
